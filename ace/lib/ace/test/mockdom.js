...
var initializers = {
    ...
    canvas: function () {
        this.getContext = function (contextId, options) {
            if (this._contextMock !== undefined)
                return this._contextMock;
...
function parseSimpleSelector(selector) {
    var tests = [];
    selector.replace(/([#.])?([\w-]+)|\[\s*([\w-]+)\s*(?:=\s*["']?([\w-]+)["']?\s*)?\]|\*|./g, function (_, hash, name, attr, attrValue) {
        if (hash == "#") {
...
(function () {
    this.initMouseEvent = function (type, _1, _2, window, detail, x, y, _x, _y, ctrl, alt, shift, meta, button, relatedTarget) {
        this.bubbles = true;
        this.type = type;
...
exports.loadInBrowser = function (global) {
    ...
    for (var i in window.document) {
        ...
        if (typeof val == "function") {
            ...
            Object.defineProperty(global.document, i, {
                value: val,
                enumerable: true,
                configurable: true
            });
