...
function checkModes() {
    ...
    jsFileList(cwd + "../../snippets").forEach(function (snippetFileName) {
        if (/\.snippets$/.test(snippetFileName))
            return;
    ...
    function testNextState(tokenizer, modeName) {
        var keys = Object.keys(tokenizer.states);
        for (var i = 0; i < keys.length; i++) {
            var key = keys[i];
            var tokens = tokenizer.states[key];
    ...
    function checkState(stateNames, token, property, modeName) {
        if (token.hasOwnProperty(property) && typeof token[property] === "string" && !stateNames.includes(token[property])) {
    ...
    Object.keys(states || {}).forEach(function (state) {
        states[state].forEach(function (rule) {
            ...
            if (result.status != "safe") {
                if (result.attack && result.attack.string)
                    delete result.attack.string;
